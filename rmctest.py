...
saber1 = Sabertooth('/dev/ttyACM2', baudrate=115200, address=128, timeout=1)
saber2 = Sabertooth('/dev/ttyACM3', baudrate=115200, address=128, timeout=1)
lift =	serial.Serial('/dev/ttyACM0')
deploy = serial.Serial('/dev/ttyACM1')



motor_ports = [port for port in myports if '/dev/ttyACM0' in port if '/dev/ttyACM1' in port  if '/dev/ttyACM2' in port   if '/dev/ttyACM3' in port ] 


def check_presence(correct_port, interval=0.1):
	while True:
		myports = [tuple(p) for p in list(serial.tools.list_ports.comports())]
		if motor_ports not in myports:
			print "Motor Controllers Disconneted"
			break
		time.sleep(interval)
...
while done==False:
    ...
    for i in range(joystick_count):
     ...
     #       axis3=axis3*.5	
	    ...
	    hat0 = joystick.get_hat(0) == (-1,0)
		#hat1 = joystick.get_hat(1)
		#hat2 = joystick.get_hat(2)
		hat1 = joystick.get_hat(0) == (0,1)
...
port_controller = threading.Thread(target=check_presence, args=(motor_ports, 0.1))
